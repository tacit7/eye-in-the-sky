...
func (tb *TableBuilder) RenderTable(rows [][]string) (string, int) {
	...
	height := 0
	leftPadding := "  " // Add 2 spaces of left padding

	// Render header
	header := tb.renderHeader()
	if header != "" {
		result.WriteString(leftPadding + header)
		result.WriteString("\n")
		height++
	}
	...
	if tb.BorderStyle != BorderNone {
		...
		result.WriteString(leftPadding + separator)
		result.WriteString("\n")
		height++
	}
	...
	for i, row := range rows {
		...
		result.WriteString(leftPadding + renderedRow)
		if i < len(rows)-1 {
			result.WriteString("\n")
		}
		height++
